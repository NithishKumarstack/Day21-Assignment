...
const Cart = (props) => {
    ...
    return (
       <div className='cart'>
        ...
        <div>{cartItems.some((item) => item.id === list.id) ? (<button className='remove' onClick={addingItemToCart(list)}>Remove From Cart</button>) : (<button className='add' onClick={addingItemToCart(list)}>Add To Cart</button>) }</div>
       </div>
    );
};
